...
export default function RootLayout({ children }) {
  return (
    <html lang="en" className={`${inter.variable} ${outfit.variable}`}>
      <body className="min-h-screen w-full flex flex-col bg-black text-white font-[var(--font-body)] antialiased overflow-x-hidden">
        <LoadingScreen />
        <ScrollProgress />
        <SpaceBackground />
        <Navbar />
        <main className="relative z-10 flex-1 w-full flex flex-col items-center">{children}</main>
        <Footer />
        <WhatsAppButton />
      </body>
    </html>
