...
export default function BrandSolarSystem() {
  ...
  const [stats, setStats] = useState([
    { id: 's1', val: 0, target: 25, suffix: '+', label: 'Projects' },
    { id: 's2', val: 0, target: 300, suffix: '%', label: 'Avg Growth' },
    { id: 's3', val: 0, target: 10, suffix: '+', label: 'Industries' },
    { id: 's4', val: 0, target: 98, suffix: '%', label: 'Satisfaction' },
  ]);

  useEffect(() => {
    if (!isStatsInView) {
      setStats(prev => prev.map(s => ({ ...s, val: 0 })));
      return;
    }
  ...
  return (
    <div ref={containerRef} className="brand-solar-container relative w-full h-[380px] sm:h-[480px] md:h-[600px] lg:h-[800px] flex items-center justify-center overflow-hidden">
      ...
      {/* Stat Bar Overlay */}
      <div ref={statsRef} className="stat-bar absolute bottom-4 md:bottom-10 left-0 right-0 flex justify-center gap-4 sm:gap-6 md:gap-16 pointer-events-none z-10 animate-[fadeIn_2.5s_ease_1s_forwards] opacity-0 px-2">
        {stats.map((stat) => (
          <div key={stat.id} className="text-center">
            <div className="text-xl sm:text-2xl md:text-4xl font-bold font-[var(--font-heading)] bg-gradient-to-br from-accent-cyan to-accent-purple bg-clip-text text-transparent leading-none">
              {stat.val}{stat.suffix}
            </div>
            <div className="text-[0.55rem] sm:text-[0.6rem] md:text-xs text-white/40 uppercase tracking-[1px] sm:tracking-[2px] mt-1 md:mt-2">
              {stat.label}
            </div>
          </div>
        ))}
